...
std::unique_ptr<Path> VisPlanner::run(PlanRequest& planRequest) {
    ...
    if (shouldReplan(planRequest, actualDynamic, &debugOut)) {
        auto path = generateVisPath(start, goal, motionConstraints, obstacles,
...
std::unique_ptr<InterpolatedPath> VisPlanner::generateVisPath(
    ...
    unsigned shellID) {
    const int tries = 1; //I have changed tries to 1, because it should only really need one
...
vector<Point> VisPlanner::runRRTHelper(
...
vector<CubicBezierControlPoints> VisPlanner::generateCubicBezierPath(
    ...
    VectorXd solutionX =
        VisPlanner::cubicBezierCalc(vi.x(), vf.x(), pointsX, ks, ks2);
    VectorXd solutionY =
...
std::unique_ptr<Planning::InterpolatedPath> VisPlanner::generateCubicBezier(
    const std::vector<Point>& points, const ShapeSet& obstacles,
    const MotionConstraints& motionConstraints, Point vi, Point vf) {
...
VectorXd VisPlanner::cubicBezierCalc(double vi, double vf,
